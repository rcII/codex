...
CCH history checkpoint, page, and finalize messages require an exact `snapshotSha256` acknowledgement. It is lowercase SHA-256 over the compact UTF-8 JSON bytes `{"historyRevision":<string>,"preludeSha256":<string>,"sourceHighWaterOrdinal":<unsigned integer>}` in that field order, binding each capture to one source-owned rollout prefix and prelude epoch.
